...
- [ ] Search for `Time:` field in commit bodies (from git-commit)
...
In the generated file each project name links to its own file —
`active/<name>.md` while active, `archived/<name>.md` once archived.

```markdown
# Project Portfolio - Dynamic List
*Auto-generated by projects*

## Active Projects (X/10)

| Pos | Project | Last Updated | Status |
|-----|---------|--------------|--------|
| 1 | **Project Name** | [Date] | [Brief status] |
| 2 | ... | ... | ... |
...
| Project | Status |
|---------|--------|
| **Project Name** | Archived (LRU) - [Date] |
...
- Source: git-commit's SESSION CONTEXT section
...
4. **Duration tracking** -- parse `Time:` from git-commit messages, accumulate per project
...
| No git-commit time data available | Use `~XX min` estimate or ask user |
...
| **session-log** | Diary captures daily narrative; project captures project-specific progress |
...
- **Lv.2** -- Duration Tracking: Parse git-commit time estimates, accumulate per project, progressive display format. Line Limit Enforcement: 1000-line cap with auto-summarization.
