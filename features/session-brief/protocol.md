...
|------|---------|----------|
| `memory/session.md` | Last session recap | Yes |
| `memory/reminders.md` | Open reminder items | Optional |
...
*Requires time-aware. If not installed, skip the work suggestion line.*
...
- *Requires projects. If not installed, skip this section.*
...
Last session: [1–2 line recap from session.md]
...
- "Last session" is the only mandatory line (requires `session.md`)
...
If no companion systems are installed, deliver a minimal brief using only `session.md`:

```
📋 Session Brief

Last session: [recap from session.md]
```
...
- If using projects with project tiers, apply tier-specific thresholds instead of the defaults
